...
- Workforce administration endpoints SHALL resolve the validated Cognito subject to the current HIS application user and evaluate current database-backed memberships, role assignments, permission grants, validity periods, and organization scope on every request. The frontend SHALL NOT be trusted to assert administrative scope. The initial workforce-directory slice MAY reconcile read-only Cognito account status with HIS membership records, but a missing identity binding or permission SHALL fail closed and Cognito groups SHALL remain irrelevant to authorization.
