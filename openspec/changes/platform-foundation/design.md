...
- The staging browser sign-in flow SHALL use Cognito SRP and support the administrator-issued temporary-password, software-token enrollment, and software-token challenge states. Cognito SHALL supply the software-token secret; the browser MAY render a standard `otpauth://` URI as a QR code locally but SHALL NOT send or persist that secret outside the active enrollment flow. The browser application SHALL retain only the 15-minute access token in application memory, SHALL clear Cognito's refresh and ID token cache after authentication, and SHALL require reauthentication when that access token expires. It SHALL NOT persist workforce tokens in local storage, session storage, or browser-delivered configuration.
