...
- [ ] 3.5 Implement a POC patient identity boundary separate from workforce identities, using native email sign-in and an auditable explicit portal-profile link with no automatic email or phone merging and no clinical-record access.
- [ ] 3.5a Implement a synthetic-data basic patient appointment portal where a linked patient can view, request, cancel, or reschedule only their own appointments without clinical, payment, messaging, or advanced identity-proofing features.
